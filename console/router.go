...
func addSession(id string, s Sessions) {
	sessionsMu.Lock()
	defer sessionsMu.Unlock()
	session[id] = s
}

func getSession(id string) Sessions {
	sessionsMu.RLock()
...
func ExecRouter(disable bool, con *ReqCreateExec) *mux.Router {
	// 路由
	mux0 := mux.NewRouter()
	...
	mux0.HandleFunc("/create_exec", func(w http.ResponseWriter, r *http.Request) {
		...
		*req = *con
		...
		// 获取驱动
		sesss, err := GetDrivers(req.Name, req.Host)
		if err != nil {
			ResponseJSON(w, http.StatusBadRequest, errMsg{err.Error()})
			return
		}
